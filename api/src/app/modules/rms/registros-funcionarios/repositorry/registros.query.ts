import { RegistrosFuncionariosSearchDto } from '../dtos';
...
to_char(pn.fecha_nacimiento,'YYYY-MM-DD') as fecha_nacimiento_format_asfi,
...
to_char(rf.fecha_ingreso,'YYYY-MM-DD') as fecha_ingreso_format_asfi,
...
export const titleHeader = [
  ...
  {col:'B1',size:30,name:'nro_identificacion',label:'Documento de Identidad'},
  ...
  {col:'G1',size:40,name:'estado',label:'Estado'}
]
...
export const whereQuery={
  ...
  export const findBuild = (search:RegistrosFuncionariosSearchDto) => {
    const where:any = {};
    where.status=true    
    if(search.nroIdentificacion){where.personaNatural={nroIdentificacion : search.nroIdentificacion}}
    if(search.nombres){where.personaNatural={nombres : search.nombres}}
    if(search.apellidos){where.personaNatural={primerApellido : search.apellidos}}
